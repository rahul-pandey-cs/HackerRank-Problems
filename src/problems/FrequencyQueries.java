...
public class FrequencyQueries {
	...
	static List<Integer> freqQuery(List<List<Integer>> queries) {
		...
		for (List<Integer> input : queries) {
			if (input.get(0) == 1)
				frequencyMap.put(input.get(1), frequencyMap.containsKey(input.get(1)) ? frequencyMap.get(input.get(1)) + 1 : 1);
			else if (input.get(0) == 2)
				frequencyMap.put(input.get(1), frequencyMap.containsKey(input.get(1)) ? frequencyMap.get(input.get(1)) - 1 : 0);

			else if (input.get(0) == 3) {
				boolean flag = false;
//				frequencyMap.forEach((key, value) -> {     // Can't use lambda expression as flag should be final
//					if (value == input.get(1)) {
//						flag = true;
//					}
//				});
				System.out.println("Frequency Map value: " + frequencyMap);
				for (Map.Entry<Integer, Integer> entry : frequencyMap.entrySet()) {
					if (entry.getValue() == input.get(1)) {
						flag = true;
					}
				}
				if (flag)
