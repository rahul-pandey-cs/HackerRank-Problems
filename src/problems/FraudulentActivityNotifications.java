...
public class FraudulentActivityNotifications {
	...
	static int activityNotifications(int[] expenditure, int d) {
		...
		for (int i = d; i < expenditure.length; i++) {
			if (expenditure[i] > 2 * calculateMedian(Arrays.copyOfRange(expenditure, i - d, i - 1), d)) {
				fraudNotifications++;
			}
		}
	...
	static int calculateMedian(int[] splitExpenditure, int d) {
		int median = 0;
		Arrays.sort(splitExpenditure);
		if (splitExpenditure.length % 2 == 0)
			median = (splitExpenditure[splitExpenditure.length / 2] + splitExpenditure[splitExpenditure.length / 2 - 1])
					/ 2;
		else
			median = splitExpenditure[splitExpenditure.length / 2];
		return median;
	}
